...
func TestCase(t *testing.T) {
	...
	name := func() string {
		s := caseingShuffle("library/model:tag")
		return fmt.Sprintf("%s/%s", addr, s)
	}
	...
	checkManifestList := func() {
		...
		var got []string
		t.Logf("got manifests:")
		err := filepath.Walk(mandir, func(path string, info fs.FileInfo, err error) error {
			if info.IsDir() {
				return nil
			}
